...
|------------|-----------|-------------|
| **base** | Always after sphere_overview | 67 tools: navigation, geometry, basic analysis |
...
open_sphere(path) → Phase 2 (23 tools: always + gateway + edge)
  ...
  └─ sphere_overview()     → Phase 3 (90-106 tools: full manual mode)
    ↓
...
| **Tool Annotations** | `readOnlyHint=True` on 67 read-only tools | Host assumes worst case |
...
| Phase 3 — full sphere (all capabilities) | ~106 | ~7k tk (filtered + trimmed docstrings) |
| Phase 3 — simple sphere (base only) | ~90 | ~5k tk |

Without 3-phase loading, all 106 tool schemas would be in context from the start (~22k tk).
