...
|------------|-----------|-------------|
| **base** | Always after sphere_overview | 63 tools: navigation, geometry, basic analysis |
...
open_sphere(path) → Phase 2 (23 tools: always + gateway + edge)
  ...
  └─ sphere_overview()     → Phase 3 (91-101 tools: full manual mode)
    ↓
...
| Phase 3 — full sphere (all capabilities) | ~101 | ~7k tk (filtered + trimmed docstrings) |
| Phase 3 — simple sphere (base only) | ~67 | ~5k tk |

Without 3-phase loading, all 101 tool schemas would be in context from the start (~22k tk).
