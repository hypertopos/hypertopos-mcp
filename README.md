...
**hypertopos-mcp** gives AI agents a safe, stateful way to explore Geometric Data Spheres without writing SQL or touching the storage layer. It wraps the [hypertopos](https://github.com/hypertopos/hypertopos-py) core library into 55 MCP tools with automatic context management, 3-phase tool visibility, and smart detection recipes that chain navigation primitives into end-to-end investigative workflows.
...
| **Phase 2** | `open_sphere` | 5 tools (Phase 1 + `detect_pattern`, `sphere_overview`) |
| **Phase 3** | `sphere_overview` | 40-55 tools (full manual toolset, capability-dependent) |
...
| Hub & Network | 6 | Hubs (π7), neighborhood, counterparties, chains |
| Temporal | 2 | Drift (π9), trajectory similarity (π10) |
| Risk Profiling | 4 | Cross-pattern profile, composite risk, passive scan |
| Detection Recipes | 5 | `detect_pattern` meta-tool + 4 single-call detection recipes |
